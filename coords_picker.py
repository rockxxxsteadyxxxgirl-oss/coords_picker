...
<head>
  ...
  <style>
    :root {
      --bg: linear-gradient(180deg, #6bb9ff 0%, #9fd7ff 45%, #e8f7ff 100%);
      --fg: #1f2937;
...
<body>
  ...
  <div class="page">
    ...
    <div class="panel">
      ...
      <div class="row" style="display:block;">
        ...
        <ul class="guide">
          <li>地図をタップすると座標と地名を取得し、全サイトボタンが有効になります。現在地ボタンでも同じ処理を行います。</li>
          <li>座標入力欄に「緯度,経度」（例: 35.0, 139.0）を入れて「この座標へ移動」で任意地点を指定できます。</li>
          <li>サイトボタンを押すと各サービスを新規タブで開きます。Windy(JMA MSM)のみデフォルトズームを広めに設定しています。</li>
          <li>Windy 3分割ボタンは別ウィンドウで ECMWF / GFS / ICON / SCW を同時表示します（ポップアップブロックを解除してください）。</li>
          <li>お気に入りは最大30件まで保存・並べ替え・削除が可能。名前未入力なら地名→座標の順で自動設定します。</li>
          <li>メモ欄・お気に入り・サイトの並び順・テーマはブラウザのローカルストレージに保存され、再訪時に復元されます。</li>
          <li>ライト/ダーク切替はページ上部のボタンで切替。サイトボタンの並び替えはドラッグ＆ドロップで行えます。</li>
        </ul>
      </div>
      <div class="row">
  ...
  <script>
    ...
    const DEFAULT_ZL = "13";
    const WINDY_Z = 10;
    const WINDY_JMA_Z = 11; // MSMはやや広域をデフォルト表示
